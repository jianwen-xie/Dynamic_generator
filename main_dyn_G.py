...
tf.flags.DEFINE_integer('num_epochs', 3000, 'Number of epochs')
tf.flags.DEFINE_integer('num_frames', 60, 'number of frames used in training data')
...
tf.flags.DEFINE_integer('motion_type_size', 0, 'motion type dimension') # please always set this to be 0.
...
tf.flags.DEFINE_string('category', 'fire', 'name of category')
tf.flags.DEFINE_string('data_path', './trainingVideo/dynamicTexture', 'path of the training data')
...
def main():
    with tf.Session() as sess:
        ...
        if CONFIG.isTraining:
            ...
            model.test(ckpt_path, appearance_path, motion_path, state_initial_path, is_random_content=True, is_random_motion_type=True, is_random_state_initial=False)
